...
Availability | Limited by the availability of the single node and it’s network connection. | Multiple replicas; failover needs to be coordinated | Sharding may lower overall availability - anyone unavailable shard may make the DB unavailable. | Availability based on the number of replicas 
...
Rook provides an operator to deploy and manage CockroachDB and YugabyteDB clusters.

Another CNCF storage project [Vitess](https://vitess.io) also provides an operator to manage MySQL clusters.
